...
import { withrawalURL } from '../dataUrl';
...
function Withrawal() {
    ...
    const HandleClick = async () => {
        try {
            ...
            const data = await axios.post(withrawalURL, {
                amount: parseInt(amountRef.current.value),
            }, { headers });
            console.log(data.data.token);
            setErr("המשיכה בוצעה בהצלחה")
            setLoading(false);
        }
        catch (err) {
    ...
    return (
        <div>
            ...
            <div className=" shadow-lg rounded-3xl mt-[10%] overflow-y-hidden overflow-hidden ml-auto mr-auto  h-[200px] w-[360px] bg-customBlue">
                ...
                <h2 className='text-white items-start text-2xl  text-center mt-2'>משיכה בנקאית</h2>
